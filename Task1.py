...
class cross_zeros():
    ...
    def game():
        ...
        while count <= 9:
            print(
                f'Player {player_number} enters coordinates (x, y): ', end='')
            x, y = coordinates.check_coordinates()
            x_list.append(x)
            y_list.append(y)
            move = (x, y)
...
def main():
    ...
    table_game = cross_zeros.game()
    print(cross_zeros.victory_check(table_game))
